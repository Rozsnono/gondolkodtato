...
export default function Login({ onClose }: { onClose: () => void }) {
    ...
    return (
        <main className="fixed top-0 left-0 w-screen h-screen bg-black/30 flex justify-center lg:items-center items-start p-4 z-[1001]">
            <div className="flex flex-col lg:w-1/4 w-full rounded-lg border border-slate-400 bg-slate-900 p-4 relative">
                {
                    formType === 'login' ? (
...
function LoginForm({ changeForm }: { changeForm: (form: 'login' | 'register') => void }) {
    ...
    return (
        <form className="flex-1 px-4 py-6" onSubmit={handleSubmit}>
            <div className="mb-4">
                <label htmlFor="name" className="block text-sm font-medium text-slate-100">Name</label>
            ...
            <button type="submit" className="w-full rounded-md bg-slate-400 p-2 text-slate-900 hover:bg-slate-300 cursor-pointer">Sign In</button>

            <div className="mt-4">
                <p className="text-sm text-slate-400">Do not have an account? <a href="#" onClick={() => changeForm('register')} className="text-slate-100 hover:underline">Sign up</a></p>
            </div>
        </form>
    )
}
...
function RegisterForm({ changeForm }: { changeForm: (form: 'login' | 'register') => void }) {
    ...
    return (
        <form className="flex-1 px-4 py-6" onSubmit={handleSubmit}>
            ...
            <button type="submit" className="w-full rounded-md bg-slate-400 p-2 text-slate-900 hover:bg-slate-300 cursor-pointer">Sign Up</button>

            <div className="mt-4">
                <p className="text-sm text-slate-400">Do you have an account? <a href="#" onClick={() => changeForm('login')} className="text-slate-100 hover:underline">Sign in</a></p>
            </div>
        </form>
    )
}