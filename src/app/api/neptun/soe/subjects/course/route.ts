...
export async function GET(req: Request) {
    ...
    try {
        ...
        const subjects = await fetch(`https://neptun3r.nyme.hu/hallgato/api/SubjectApplication/GetSubjectsCourses?termId=${termId}&subjectId=${subjectId}&curriculumTemplateId=${curriculumId}&curriculumTemplateLineId=${curriculumLineId}`,
            {
                method: 'GET',
                headers: {
        ...
        const results = await Promise.all(
            data.map(async (item: any) => {
                ...
                const response = await fetch(
                    `https://neptun3r.nyme.hu/hallgato/api/SubjectApplication/SchedulableSubjects?request.termId=${termId}&request.subjectType=${subjectType}&request.hasRegisteredSubjects=true&request.hasScheduledSubjects=true&request.curriculumTemplateId=${curriculumId}&request.subjectGroupId=${subjectGroupId}&sortAndPage.title=asc&request.title=${title}`,
                    {
                        method: 'GET',
                        headers: {
                            'Authorization': `${authToken}`,
                ...
                const courses = await fetch(`https://neptun3r.nyme.hu/hallgato/api/SubjectApplication/GetSubjectsCourses?termId=${data.termId}&subjectId=${data.id}&curriculumTemplateId=${data.curriculumTemplateId}&curriculumTemplateLineId=${data.curriculumTemplateLineId}`,
                    {
                        method: 'GET',
                        headers: {
                            'Authorization': `${authToken}`,
