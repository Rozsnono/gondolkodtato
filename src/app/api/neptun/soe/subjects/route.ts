...
export async function GET(req: Request) {
    ...
    try {
        ...
        const subjects = await fetch(`https://neptun3r.nyme.hu/hallgato/api/SubjectApplication/SchedulableSubjects?request.termId=${termId}&request.subjectType=${subjectType}&request.hasRegisteredSubjects=true&request.hasScheduledSubjects=true&request.curriculumTemplateId=${curriculumId}&request.subjectGroupId=${subjectGroupId}&sortAndPage.firstRow=${from}&sortAndPage.lastRow=${to}&sortAndPage.title=asc${title ? `&request.title=${title}` : ''}`,
            {
                method: 'GET',
                headers: {
